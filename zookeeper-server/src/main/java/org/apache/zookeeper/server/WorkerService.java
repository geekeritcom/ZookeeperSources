 ...
 *
 * WorkerService是一个worker线程池，用于运行任务，使用一个或者多个ExecutorServices实现。
 * 支持不同类型的线程：
 *      1.可分配的线程，创建多个独立的单个线程的ExecutorServices。
 *      {@link org.apache.zookeeper.server.quorum.CommitProcessor}使用一个可分配的WorkerService，
 *      因为一个给定session的请求必须顺序执行
 *      2.不可分配的线程，创建一个包含多个线程的ExecutorServices
 *      {@link NIOServerCnxnFactory}使用未分配的WorkerService
 *      因为所有的socket IO请求都是顺序独立的，允许ExecutorService来处理线程分配，用于优化性能
 *
 *
 */
public class WorkerService {
     ...
     *
     * 创建worker线程池的线程工厂，自定义实现的目的如下：
     *  1.希望更方便的定义worker线程的名称
     *  2.设置所有的worker线程为守护线程，这样可以在服务关闭时不阻塞服务器
     */
    ...
    public void start() {
        // 显示指定工作线程数量为0时表示不创建对应的线程池，而是直接使用主处理请求的线程来处理
        // zookeeper处理请求快速的原因也在于一般不使用主处理请求线程来处理请求，而是将请求分发给对应的线程池处理
