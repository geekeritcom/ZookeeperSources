...
public class FastLeaderElection implements Election {
     ...
     * <p>
     * 通知消息用来使集群中其他机器知道给定机器已经改变了他的投票，可能是由于
     * 给定机器加入了选举过程或者已经得知另外的机器拥有更高的事务ID或者事务ID相同，
     * 但是另一台机器的ID值更大
     */
     ...
     * <p>
     * 主机想要发送给其他主机的信息，可以是通知事件消息或者收到事件通知的ACK消息
     */
     ...
     * spawns a new thread.
     * <p>
     * 消息处理器的多线程的实现，有两个子类：WorkReceiver与WorkSender
     * 用于接收消息与发送消息，两个对应的处理类实例都会产生新的线程处理。
     */
    ...
    protected class Messenger {
            ...
            public void run() {
                ...
                while (!stop) {
                    ...
                    try {
                         ...
                         *
                         * 接受到的消息来自observer或者非投票的follower时，直接返回消息
                         */
         ...
         * <p>
         * 每个实例维护一个消息发送线程和一个消息接收线程，这两个线程都被设置为守护线程
         *
     ...
     * longer leading.
     * <p>
     * 当leader被选举出来并且被一个quorum支持时，需要检查该leader是否投票并且处于leading状态
     * <p>
     * 需要检查是由于避免其他服务器不断选举另外的已经崩溃且无法继续leading的服务器
     *
     * @param votes         set of votes
     ...
     * role of the server.
     * <p>
     * 在集群中leader被选举出来后，根据节点的类型来确定当前服务应该处于何种状态
     *
     * @return ServerState
     */
    ...
    private void setPeerState(long proposedLeader, SyncedLearnerTracker voteSet) {
        // 当前节点如果被选为leader节点，那么服务状态应该为leading
     ...
     * <p>
     * 开启新一轮的leader选举，当QuorumPeer的状态变为LOOKING状态时就会调用该方法，
     * 随后发送通知事件给其他所有的服务器
     */
    public Vote lookForLeader() throws InterruptedException {
        ...
        try {
            /*
             * The votes from the current leader election are stored in recvset.
             * In other words, a vote v is in recvset
             * if v.electionEpoch == logicalclock.
             ...
             *
             * 在节点没有找到leader之前将一直从接收队列中获取其他节点发送的通知消息来处理
             */
